...
def login_api(username: str, password: str):
    return request_json(
        "POST", "/login", data={"username": username, "password": password}, auth=False
    )


def register_api(payload: dict):
    return request_json("POST", "/register", json=payload, auth=False)
...
def create_user_admin_api(payload: dict):
    return request_json("POST", "/users", json=payload)


def update_user_role_api(user_id: int, role: str):
    return request_json("PUT", f"/users/{user_id}/role", json={"role": role})


def reset_user_password_api(user_id: int, new_password: str):
    return request_json(
        "PUT", f"/users/{user_id}/password", json={"new_password": new_password}
    )


def change_own_password_api(current_password: str, new_password: str):
    return request_json(
        "PUT",
        "/me/password",
        json={
...
def update_user_status_api(user_id: int, is_active: bool):
    return request_json(
        "PUT", f"/users/{user_id}/status", json={"is_active": is_active}
    )
...
def login_or_register_ui():
    st.title("🔐 ProTrack Access")
    st.markdown("Create an account or log in to continue.")
    ...
    with login_tab:
        ...
        if submit:
            response, success, error = login_api(username.strip(), password)
            if success:
                st.session_state["access_token"] = response["access_token"]
                st.session_state["username"] = response["username"]
                st.session_state["role"] = response["role"].lower()
    ...
    with register_tab:
        role = st.selectbox(
            "Role", ["user", "superuser", "admin"], key="public_register_role"
        )
        ...
        if role in ["superuser", "admin"]:
            role_code = st.text_input(
                "Role Access Code",
                type="password",
                help="Required for superuser or admin registration",
                key="public_register_role_code",
            )
        ...
        if submit:
            if not username.strip():
                ...
                st.error("Password too long. Please use 128 characters or fewer.")
                ...
                payload = {
                    ...
                    "password": password,
                    "role": role.lower(),
                    "role_code": role_code if role in ["superuser", "admin"] else None,
                }
                response, success, error = register_api(payload)
                if success:
...
def check_api_connection():
    response, success, error = request_json("GET", "/health", auth=False)
    if not success:
...
def create_project_api(project):
    return request_json("POST", "/projects/", json=project)


def get_all_projects():
    data, success, error = request_json("GET", "/projects/")
    return (data, success) if success else (None, False)


def get_project_by_db_id(db_id):
    data, success, error = request_json("GET", f"/projects/id/{db_id}")
    return (data, success) if success else (None, False)


def get_projects_by_name_keyword(keyword):
    data, success, error = request_json("GET", f"/projects/search/{keyword}")
    return (data, success) if success else (None, False)


def get_projects_by_sid(sid):
    data, success, error = request_json("GET", f"/projects/sid/{sid}")
    return (data, success) if success else (None, False)


def get_projects_by_segment(segment):
    data, success, error = request_json("GET", f"/projects/segment/{segment}")
    return (data, success) if success else (None, False)


def get_projects_by_type(p_type):
    data, success, error = request_json("GET", f"/projects/type/{p_type}")
    return (data, success) if success else (None, False)


def get_projects_by_ta_id(ta_id):
    data, success, error = request_json("GET", f"/projects/ta_id/{ta_id}")
    return (data, success) if success else (None, False)


def get_projects_by_job_id(job_id):
    data, success, error = request_json("GET", f"/projects/job_id/{job_id}")
    return (data, success) if success else (None, False)


def get_projects_by_job_ol_id(job_ol_id):
    data, success, error = request_json("GET", f"/projects/job_ol_id/{job_ol_id}")
    return (data, success) if success else (None, False)


def get_projects_by_job_ra_id(job_ra_id):
    data, success, error = request_json("GET", f"/projects/job_ra_id/{job_ra_id}")
    return (data, success) if success else (None, False)


def get_projects_by_status(status):
    data, success, error = request_json("GET", f"/projects/status/{status}")
    return (data, success) if success else (None, False)


def get_projects_by_manager(manager):
    data, success, error = request_json("GET", f"/projects/manager/{manager}")
    return (data, success) if success else (None, False)


def get_projects_by_business_unit(b_unit):
    data, success, error = request_json("GET", f"/projects/business_unit/{b_unit}")
    return (data, success) if success else (None, False)


def get_projects_by_business_country(b_country):
    data, success, error = request_json(
        "GET", f"/projects/business_country/{b_country}"
    )
    return (data, success) if success else (None, False)


def get_projects_by_business_name(b_name):
    data, success, error = request_json("GET", f"/projects/business_name/{b_name}")
    return (data, success) if success else (None, False)


def get_projects_by_margin_band(f_margin_band):
    data, success, error = request_json("GET", f"/projects/margin_band/{f_margin_band}")
    return (data, success) if success else (None, False)


def get_projects_by_date(month=None, year=None, since=None, date_type="start"):
    params = {}
    if month:
        params["month"] = month
    if year:
    ...
    data, success, error = request_json("GET", "/projects/filter/date", params=params)
    return (data, success) if success else (None, False)


def update_project_by_db_id(db_id, updates):
    return request_json("PUT", f"/projects/{db_id}", json=updates)


def delete_project_by_db_id(db_id):
    data, success, error = request_json("DELETE", f"/projects/{db_id}")
    if success:
        return data, True
    return None, False
...
def main():
    ...
    tab_funcs = [show_dashboard, create_project_ui, search_projects, ai_insights]

    if role in ["user", "superuser", "admin"]:
        tab_names.insert(3, "✏️ Update Project")
        tab_funcs.insert(3, update_project)

    if role in ["user", "superuser", "admin"]:
        tab_names.insert(4, "🗑️ Delete Project")
        tab_funcs.insert(4, delete_project)

    if role in ["user", "superuser", "admin"]:
        tab_names.append("⚙️ Admin Settings")
        tab_funcs.append(admin_settings_ui)
...
def show_dashboard():
    ...
    with col_right:
        ...
        s2.metric("Low Margin", len(low_margin_projects))
...
def create_project_ui():
    ...
    with st.form("create_project_form"):
        ...
        with col8:
            ...
            if f_revenue is not None and f_cost is not None:
                ...
                if auto_nprofit < 0:
                    st.warning(f"⚠️ Loss-making project: {f_currency} {auto_nprofit}")

                if float(f_revenue) != 0:
                    auto_margin = round((auto_nprofit / float(f_revenue)) * 100, 2)
                else:
                    auto_margin = 0.0
                ...
                if auto_margin <= 0:
                    ...
                    auto_margin_band = "1-19%"
                elif auto_margin < 50:
                    auto_margin_band = "20-49%"
                elif auto_margin < 80:
                    auto_margin_band = "50-79%"
                else:
                    auto_margin_band = "80-100%"

            st.text_input(
                "Fieldwork Net Profit",
                value=str(auto_nprofit) if auto_nprofit is not None else "",
                placeholder="Auto-calculated from revenue and cost",
                disabled=True,
            )

            st.text_input(
                "Profit Margin (%)",
                value=f"{auto_margin}%" if auto_margin is not None else "",
                placeholder="Auto-calculated from revenue and cost",
                disabled=True,
            )
            ...
            st.text_input(
                "Margin Band",
                value=auto_margin_band,
                placeholder="Auto-calculated from profit margin",
                disabled=True,
            )
    ...
    if submit_button:
        if not p_name or not p_name.strip():
            ...
            return
        if st.session_state.get("role") in ["superuser", "admin"]:
...
def update_project():
    ...
    with st.form(f"update_project_form_{db_id}"):
        ...
        if section_to_update == "Project Information":
            ...
            st.text_input(
                "Fieldwork Net Profit",
                value=str(auto_nprofit),
                disabled=True,
                key=f"update_auto_nprofit_{db_id}",
            )
            st.text_input(
                "Final Margin (%)",
                value=f"{auto_margin}%",
                disabled=True,
                key=f"update_auto_margin_{db_id}",
            )
            market = st.text_input(
                "Market",
        ...
        with report_tab1:
            ...
            if st.button("Generate Report 📄", key="generate_project_report"):
                try:
                    ...
                    res = requests.get(
                        f"{API_BASE_URL}/reports/projects",
                        params=params,
                        headers=get_auth_headers(),
                        timeout=30,
                    )
...
def admin_settings_ui():
    ...
    st.markdown("Manage account settings and access controls.")
    ...
    section = st.selectbox("Choose Action", section_options)

    users, success, error = get_users_api()
    visible_users = []
    if success and users:
        if role == "admin":
            visible_users = users
        elif role == "superuser":
            visible_users = [u for u in users if u.get("role") in ["user", "superuser"]]
        else:
            visible_users = [u for u in users if u.get("username") == username]

    if section == "View Users":
        if not success:
        ...
        if submitted:
            if not username_input.strip():
                ...
                response, ok, err = create_user_admin_api(
                    {
                        "username": username_input.strip(),
                        "password": password,
                        "role": create_role.lower(),
                        "role_code": (
                            role_code if create_role in ["superuser", "admin"] else None
                        ),
                    }
                )
                if ok:
        ...
        if section in [
            ...
            if role == "superuser":
                selectable_users = [u for u in visible_users if u.get("role") == "user"]
            else:
                selectable_users = visible_users
        else:
            selectable_users = visible_users
        ...
        if section in [
            ...
            if not user_labels:
                st.info("No eligible users available.")
                return

            selected_label = st.selectbox("Select User", user_labels)
            selected_user = selectable_users[user_labels.index(selected_label)]

        if section == "Update User Role":
            role_options = ["user", "superuser", "admin"]
            current_role = str(selected_user["role"]).lower()
            ...
            if st.button("Update Role"):
                response, ok, err = update_user_role_api(
                    selected_user["user_id"], new_role.lower()
                )
                if ok:
        ...
        elif section == "Reset Password":
            if role == "admin":
                reset_mode = st.radio(
                    "Reset Mode",
                    ["My Password", "Selected User Password"],
                    horizontal=True,
                )
            elif role == "superuser":
                reset_mode = st.radio(
                ...
                reset_mode = "My Password"

            if reset_mode == "My Password":
                st.subheader("🔑 Reset My Password")
                with st.form("reset_own_password_form"):
                ...
                if submitted:
                    ...
                    elif len(new_password.encode("utf-8")) > 128:
                        st.error(
                            "Password too long. Please use 128 characters or fewer."
                        )
                    elif new_password != confirm_new_password:
                        st.error("New passwords do not match.")
                    else:
                        response, ok, err = change_own_password_api(
                ...
                if st.button("Reset Password"):
                    if len(new_password.encode("utf-8")) > 128:
                        ...
                        response, ok, err = reset_user_password_api(
                            selected_user["user_id"], new_password
                        )
                        if ok:
                            st.success("Password reset successfully.")
                        else:
                            st.error(f"Failed to reset password: {err}")

        elif section == "Activate / Deactivate User":
            status_label = "Active" if selected_user["is_active"] else "Inactive"
            ...
            if st.button("Update Status"):
                response, ok, err = update_user_status_api(
                    selected_user["user_id"], new_status
                )
                if ok:
        ...
        elif section == "Delete User":
            st.warning("This action cannot be undone.")
            confirm_delete = st.checkbox(
                f"I confirm I want to delete user '{selected_user['username']}'",
            ...
            if st.button("Delete User", type="primary"):
                if not confirm_delete:
                    ...
                    response, ok, err = delete_user_api(selected_user["user_id"])
                    if ok:
