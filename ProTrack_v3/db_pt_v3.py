...
class DatabaseManager:
    def __init__(self, db_name="Project_Tracker_v3.db"):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute(
                ...
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    role TEXT NOT NULL CHECK(role IN ('user', 'superuser', 'admin')),
                    is_active INTEGER NOT NULL DEFAULT 1,
                    created_at TEXT NOT NULL
                )
            """
            )
            # Lightweight migration for existing Project_Tracker.db files
            new_project_columns = {
                "f_revenue_usd": "FLOAT",
                "f_cost_usd": "FLOAT",
                "f_nprofit_usd": "FLOAT",
            ...
            for column_name, column_type in new_project_columns.items():
    ...
    # ---------- User helpers ----------
    def create_user(self, username, password_hash, role, is_active=True):
        created_at = datetime.now(ZoneInfo("Asia/Kuala_Lumpur")).isoformat()
        query = """
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                cursor.execute(
                    query,
                    (username, password_hash, role, 1 if is_active else 0, created_at),
                )
                conn.commit()
    ...
    def create_project(self, project_data):
        query = """
            INSERT INTO projects (
                ...
                f_remarks, created_at, f_revenue_usd, f_cost_usd, f_nprofit_usd,
        ...
        values = (
            ...
            project_data.get("f_revenue_usd"),
            project_data.get("f_cost_usd"),
            project_data.get("f_nprofit_usd"),
            project_data.get("exchange_rate"),
            project_data.get("exchange_rate_source"),
            project_data.get("exchange_rate_date"),
        )
        ...
        try:
            with sqlite3.connect(self.db_name) as conn:
                ...
                conn.commit()
                print(
                    f"Project '{project_data.get('p_name')}' created successfully with DB ID: {cursor.lastrowid}"
                )
                return cursor.lastrowid
    ...
    def get_project_by_db_id(self, db_id):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_one_with_count(
                "SELECT * FROM projects WHERE db_id = ?", (db_id,)
            )
    ...
    def get_projects_by_name_keyword(self, keyword):
        with sqlite3.connect(self.db_name) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM projects WHERE p_name LIKE ? COLLATE NOCASE ORDER BY p_name",
                (f"%{keyword}%",),
            )
    ...
    def get_projects_by_s_id(self, s_id):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE s_id = ?", (s_id,)
            )
    ...
    def get_projects_by_p_segment(self, p_segment):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE p_segment = ?", (p_segment,)
            )
    ...
    def get_projects_by_p_type(self, p_type):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE p_type = ?", (p_type,)
            )
    ...
    def get_projects_by_ta_id(self, ta_id):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE ta_id = ?", (ta_id,)
            )
    ...
    def get_projects_by_job_ol_id(self, job_ol_id):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE job_ol_id = ?", (job_ol_id,)
            )
    ...
    def get_projects_by_job_ra_id(self, job_ra_id):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE job_ra_id = ?", (job_ra_id,)
            )
    ...
    def get_projects_by_p_status(self, p_status):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE p_status = ?", (p_status,)
            )
    ...
    def get_projects_by_p_manager(self, p_manager):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE p_manager = ?", (p_manager,)
            )
    ...
    def get_projects_by_b_unit(self, b_unit):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE b_unit = ?", (b_unit,)
            )
    ...
    def get_projects_by_b_country(self, b_country):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE b_country = ?", (b_country,)
            )
    ...
    def get_projects_by_b_name(self, b_name):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE b_name = ?", (b_name,)
            )
    ...
    def get_projects_by_b_status(self, b_status):
        with sqlite3.connect(self.db_name) as conn:
            ...
            return self.fetch_all_projects_count(
                "SELECT * FROM projects WHERE b_status = ?", (b_status,)
            )
    ...
    def get_projects_by_f_margin_band(self, f_margin_band):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            if f_margin_band == "0":
                ...
                return self.fetch_all_projects_count(
                    "SELECT * FROM projects WHERE f_margin = 0"
                )
            elif f_margin_band == "1%_to_19%":
                cursor.execute("SELECT * FROM projects WHERE f_margin BETWEEN 1 AND 19")
                return self.fetch_all_projects_count(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 1 AND 19"
                )
            elif f_margin_band == "20%_to_49%":
                cursor.execute(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 20 AND 49"
                )
                return self.fetch_all_projects_count(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 20 AND 49"
                )
                ...
                cursor.execute(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 50 AND 79"
                )
                return self.fetch_all_projects_count(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 50 AND 79"
                )
                ...
                cursor.execute(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 80 AND 100"
                )
                return self.fetch_all_projects_count(
                    "SELECT * FROM projects WHERE f_margin BETWEEN 80 AND 100"
                )
    ...
    def get_projects_by_date_filter(
        self, month=None, year=None, since=False, date_type="start"
    ):
        ...
        if since:
            if month is not None and year is None:
                ...
                return self.fetch_all_projects_count(
                    f'SELECT * FROM projects ORDER BY "{col}"'
                )
    ...
    def auto_complete_overdue_projects(self):
        """Set Field/Active projects to Completed once their end date has passed."""
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute(
                ...
                WHERE p_e_date IS NOT NULL
                  AND p_e_date < ?
                  AND LOWER(COALESCE(p_status, '')) IN ('field', 'active')
                """,
                (today,),
            )
            conn.commit()
    ...
    def main(self):
        while True:
            ...
            if choice == "1":
                ...
                f_deliverables = (
                    int(f_deliverables_input) if f_deliverables_input else None
                )
                ...
                if sub_choice == "4":
                    ...
                    if projects:
                        ...
                        for i, project in enumerate(projects, start=1):
                            print(
                                f"{i}. DB ID: {project[0]}, SID: {project[11]}, Project Name: {project[1]}, Project Manager: {project[2]}"
                            )

                        selected_db_id = int(
                            input("Enter the DB ID to view full project details: ")
                        )
                ...
                if sub_choice == "17":
                    ...
                    if selected_band:
                        ...
                        if projects:
                            ...
                            print(
                                f"No projects found with Final Margin in: {selected_band}"
                            )
                    else:
                ...
                if sub_choice == "18":
                    date_type = (
                        input(
                            "Filter by Start Date or End Date? (enter 'start' or 'end'): "
                        )
                        .strip()
                        .lower()
                    )
                    ...
                    year_input = input(
                        "Enter year (e.g., 2024) for filtering (optional): "
                    )
                    since_input = (
                        input(
                            "Filter projects since the specified month/year? (yes/no): "
                        )
                        .strip()
                        .lower()
                    )
                    ...
                    try:
                        projects = self.get_projects_by_date_filter(
                ...
                try:
                    db_id = int(
                        input("Enter the Database ID of the project to delete: ")
                    )

                    cursor = self.conn.cursor()
                    cursor.execute(
                        "SELECT p_name FROM projects WHERE db_id = ?", (db_id,)
                    )
                    result = cursor.fetchone()
                    ...
                    confirm = input(
                        f"Please confirm to delete project (yes/no): \n"
                        f"Database ID: {db_id} \n"
